<template>
  <div>
    <div class="grid">
      <div class="col-12">
        <h1 class="text-center">บริการด้านบัญชี</h1>
      </div>
    </div>
    <div class="grid">
      <div class="col-12 lg:col-5">
        <div class="grid">
          ...
          <div class="col-12">
            <Panel header="STEP 2. บริการด้านบัญชี">
              <AccountingList v-if="!checkOrder()" />
...
import AccountingList from '@/components/employee/pos/accounting/AccountingList.vue';
import CheckOut from '@/components/employee/pos/accounting/CheckOut.vue';
import OrderList from '@/components/employee/pos/accounting/OrderList.vue'
import CustomerDisplay from '@/components/employee/pos/CustomerDisplay.vue';
import CheckOutButton from '@/components/employee/pos/accounting/CheckOutButton.vue';
export default {
  components: {
    AccountingList, CheckOut, OrderList, CustomerDisplay,
    CheckOutButton
  },
  async created() {
    await callme(localStorage.getItem('token'));
    document.title = 'บริการด้านบัญชี'
  },
  ...
  async mounted() {
    ...
    this.$store.commit('updateTotalAccounting');
  },
  data: () => ({
    cus_name: '',
    cus_tel: '',
  ...
  methods: {
    ...
    checkOrder() {
      if (this.$store.getters.order_accounting_detail.length !== 0) {
        return true;
      } else {
        return false;
      }
