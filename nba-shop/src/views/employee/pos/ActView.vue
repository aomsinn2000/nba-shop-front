<template>
  <div>
    <div class="grid">
      <div class="col-12">
        <h1 class="text-center">บริการ พ.ร.บ. (รถยนต์ - รถจักรยานยนต์)</h1>
      </div>
    </div>
    <div class="grid">
...
import ActList from '@/components/employee/pos/act_service/ActList.vue';
import CheckOut from '@/components/employee/pos/act_service/CheckOut.vue';
import OrderList from '@/components/employee/pos/act_service/OrderList.vue'
import CustomerDisplay from '@/components/employee/pos/CustomerDisplay.vue';
import CheckOutButton from '@/components/employee/pos/act_service/CheckOutButton.vue';
export default {
  components: {
    ActList, CheckOut, OrderList, CustomerDisplay,
    CheckOutButton
  },
  async created() {
    await callme(localStorage.getItem('token'));
    document.title = 'บริการ พ.ร.บ. (รถยนต์ - รถจักรยานยนต์)'
  },
  ...
  methods: {
    ...
    checkOrder() {
      if (this.$store.getters.order_act_detail.length !== 0) {
        return true;
      } else {
        return false;
      }
