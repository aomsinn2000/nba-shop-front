<template>
  <div>
    <div class="grid">
      <div class="col-12">
        <h1 class="text-center">บริการด้านการส่งเสริมการตลาดออนไลน์</h1>
      </div>
    </div>
    <div class="grid">
      <div class="col-12 lg:col-5">
        <div class="grid">
          ...
          <div class="col-12">
            <Panel header="STEP 2. บริการด้านการส่งเสริมการตลาดออนไลน์">
              <MediaList v-if="!checkOrder()"/>
              <Message v-else :closable="false">มีรายการที่รอการชำระอยู่ กรุณาตรวจสอบและทำรายการให้เสร็จสิ้น...</Message>
            </Panel>
          </div>
        </div>
      </div>
...
import MediaList from '@/components/employee/pos/media_service/MediaList.vue';
import CheckOut from '@/components/employee/pos/media_service/CheckOut.vue';
import OrderList from '@/components/employee/pos/media_service/OrderList.vue'
import CustomerDisplay from '@/components/employee/pos/CustomerDisplay.vue';
import CheckOutButton from '@/components/employee/pos/media_service/CheckOutButton.vue';
export default {
  components: {
    MediaList, CheckOut, OrderList, CustomerDisplay,
    CheckOutButton
  },
  async created() {
    await callme(localStorage.getItem('token'));
    document.title = 'บริการด้านการส่งเสริมการตลาดออนไลน์'
  },
  ...
  async mounted() {
    ...
    await axios.get(`${process.env.VUE_APP_SHOP}shop/${localStorage.getItem('shop_id')}`, {
      ...
      const check_function = shop_function.find((el) => el.name === 'media_service');
      if (!check_function) {
        this.$toast.add({ severity: 'error', summary: 'ผิดพลาด', detail: 'ร้านค้านี้ไม่สามารถใช้งานบริการส่งเสริมการตลาดออนไลน์ได้', life: 3000 });
    ...
    this.$store.commit('updateTotalMedia');
  },
  data: () => ({
    cus_name: '',
    cus_tel: '',
  ...
  methods: {
    ...
    checkOrder() {
      if (this.$store.getters.order_media_detail.length !== 0) {
        return true;
      } else {
        return false;
      }
