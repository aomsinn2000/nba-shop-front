<template>
  <div>
    <div class="grid">
      <div class="col-12">
        <h1 class="text-center">สั่งงานออกแบบโปรแกรมและเว็บไซต์</h1>
      </div>
    </div>
    <div class="grid">
      <div class="col-12 lg:col-5">
        <div class="grid">
          ...
          <div class="col-12">
            <Panel header="STEP 2. บริการออกแบบโปรแกรมและเว็บไซต์">
              <WebsiteList v-if="!checkOrder()" />
...
import WebsiteList from '@/components/employee/pos/program/WebsiteList.vue';
import CheckOut from '@/components/employee/pos/program/CheckOut.vue';
import OrderList from '@/components/employee/pos/program/OrderList.vue'
import CustomerDisplay from '@/components/employee/pos/CustomerDisplay.vue';
import CheckOutButton from '@/components/employee/pos/program/CheckOutButton.vue';
export default {
  components: {
    WebsiteList, CheckOut, OrderList, CustomerDisplay,
    CheckOutButton
  },
  async created() {
    await callme(localStorage.getItem('token'));
    document.title = 'ออกแบบโปรแกรมและเว็บไซต์'
  },
  ...
  async mounted() {
    ...
    this.$store.commit('updateTotalWebsite');
  },
  data: () => ({
    cus_name: '',
    cus_tel: '',
  ...
  methods: {
    ...
    checkOrder() {
      if (this.$store.getters.order_website_detail.length !== 0) {
        return true;
      } else {
        return false;
      }
