            ...
            <div class="col-12">
                ...
                <div class="grid mb-0 pb-0">
                    <div class="col-12 mt-2">
                        <PrintReceipt :order="res" :receive="this.receive" className="w-full" />
                    </div>
                </div>
                <div class="grid">
    ...
    methods: {
        ...
        checkPaymentType() {
            ...
            if (this.payment_type === 'เงินโอน' || this.payment_type === 'บัตรเครดิต') {
                this.receive = this.$store.getters.order_accounting_total;
            } else {
                this.receive = null;
            }
        },
        check() {
            ...
            if (this.receive < this.$store.getters.order_accounting_total) {
            ...
            this.$confirm.require({
                ...
                accept: async () => {
                    ...
                    let address = 'ไม่มี';
                    const num = Number(this.$store.getters.order_accounting_total);
                    const del_wallet = this.partner_wallet - num;
                    const _id = this.partner._id;
                    if (this.$store.getters.accounting_cus.address !== '') {
                        address = this.$store.getters.accounting_cus.address;
                    }
                    this.change = this.receive - this.$store.getters.order_accounting_total;
                    const data = {
                        customer_tel: this.$store.getters.accounting_cus.tel_platform,
                        customer_name: this.$store.getters.accounting_cus.name,
                        customer_contact: this.$store.getters.accounting_cus.tel,
                        customer_address: address,
                        people_id: this.$store.getters.accounting_cus.people_id,
                        line_id: this.$store.getters.accounting_cus.line_id,
                        product_detail: this.$store.getters.order_accounting,
                        paymenttype: this.payment_type,
                        moneyreceive: this.receive,
                    }
                    await axios.post(`${process.env.VUE_APP_SHOP}accountservice/order`, data, {
                        headers: {
                            'auth-token': `Bearer ${this.$store.getters.token}`
                        }
                    }).then((res) => {
                    ...
                    const history = {
                        ...
                        name: `บริการด้านบัญชี ใบเสร็จ เลขที่ ${this.res.receiptnumber}`,
                        type: 'เงินออก',
                        amount: this.$store.getters.order_accounting_total,
        ...
        again() {
            ...
            this.$store.commit('setAccountingCus', cus);
            window.location.reload();
        }
    }
}
