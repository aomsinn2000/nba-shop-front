    ...
    methods: {
        ...
        checkPaymentType() {
            ...
            if (this.payment_type === 'เงินโอน' || this.payment_type === 'บัตรเครดิต') {
                this.receive = this.$store.getters.order_website_total;
            } else {
                this.receive = null;
            }
        },
        check() {
            ...
            if (this.receive < this.$store.getters.order_website_total) {
            ...
            this.$confirm.require({
                ...
                accept: async () => {
                    ...
                    //check address
                    let address = 'ไม่มี';
                    const num = Number(this.$store.getters.order_website_total);
                    ...
                    this.change = this.receive - this.$store.getters.order_website_total;
                    const data = {
                        customer_tel: this.$store.getters.website_cus.tel_platform,
                        customer_name: this.$store.getters.website_cus.name,
                        customer_contact: this.$store.getters.website_cus.tel,
                        customer_address: address,
                        people_id: this.$store.getters.website_cus.people_id,
                        line_id: this.$store.getters.website_cus.line_id,
                        product_detail: this.$store.getters.order_website,
                        paymenttype: this.payment_type,
                        moneyreceive: this.receive,
                    }
                    await axios.post(`${process.env.VUE_APP_SHOP}websiteservice/order`, data, {
                        headers: {
                            'auth-token': `Bearer ${this.$store.getters.token}`
                        }
                    }).then((res) => {
                    ...
                    const history = {
                        ...
                        name: `เขียนโปรแกรมและออกแบบเว็บไซต์ ใบเสร็จ เลขที่ ${this.res.receiptnumber}`,
                        type: 'เงินออก',
                        amount: this.$store.getters.order_website_total,
        ...
        again() {
            ...
            this.$store.commit('setWebsiteCus', cus);
            window.location.reload();
        }
    }
}
