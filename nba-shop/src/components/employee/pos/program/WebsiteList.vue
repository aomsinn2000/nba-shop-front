<template>
    <div class="grid">
        <div class="col-12">
            <DataView :value="website" paginator :rows="12">
                <template #empty>
    ...
    <Dialog v-model:visible="dialogChoose" header="รายละเอียด" :modal="true"
        ...
        <p class="mb-0 mt-0">{{ websites.name }}</p>
        <p class="mb-0 mt-3">ราคา : {{ numberDigitFormat(websites.price) }} บาท</p>
        <p class="mb-0 mt-3">{{ websites.detail }}</p>
        <div class="grid">
...
export default {
    ...
    async mounted() {
        await axios.get(`${process.env.VUE_APP_SHOP}websiteservice/list`, {
            headers: {
    ...
    methods: {
        ...
        async calFinal() {
            this.$store.commit('setLoading', false);
            this.total = this.websites.price * this.amount;

            const order = {
                packageid: this.websites._id,
                quantity: this.amount,
            }

            const detail = {
                price: this.websites.price,
                name: this.websites.name,
                amount: this.amount,
                total: this.total,
                detail: this.detail
            }

            this.website_detail = detail;
            this.website_order = order;
            this.dialogPrice = true;
        },

        async addCart() {
            this.$store.dispatch('addWebsiteDetail', this.website_detail);
            this.$store.dispatch('addWebsiteOrder', this.website_order);
