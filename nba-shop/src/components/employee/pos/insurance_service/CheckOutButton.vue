    ...
    methods: {
        ...
        checkPaymentType() {
            ...
            if (this.payment_type === 'เงินโอน' || this.payment_type === 'บัตรเครดิต') {
                this.receive = this.$store.getters.order_insurance_total;
            } else {
                this.receive = null;
            }
        },
        check() {
            ...
            if (this.receive < this.$store.getters.order_insurance_total) {
            ...
            this.$confirm.require({
                ...
                accept: async () => {
                    ...
                    let address = 'ไม่มี';
                    const num = Number(this.$store.getters.order_insurance_total);
                    const del_wallet = this.partner_wallet - num;
                    const _id = this.partner._id;
                    if (this.$store.getters.insurance_cus.address !== '') {
                        address = this.$store.getters.insurance_cus.address;
                    }
                    this.change = this.receive - this.$store.getters.order_insurance_total;
                    const data = {
                        customer_tel: this.$store.getters.insurance_cus.tel_platform,
                        customer_name: this.$store.getters.insurance_cus.name,
                        customer_contact: this.$store.getters.insurance_cus.tel,
                        customer_address: address,
                        people_id: this.$store.getters.insurance_cus.people_id,
                        line_id: this.$store.getters.insurance_cus.line_id,
                        product_detail: this.$store.getters.order_insurance,
                        paymenttype: this.payment_type,
                        moneyreceive: this.receive,
                        picture: this.$store.getters.insurance_img,
                    }
                    await axios.post(`${process.env.VUE_APP_SHOP}insuranceservice/order`, data, {
                        headers: {
                            'auth-token': `Bearer ${this.$store.getters.token}`
                        }
                    }).then((res) => {
                    ...
                    const history = {
                        ...
                        name: `ประกัน (รถยนต์ - รถจักรยานยนต์) ใบเสร็จ เลขที่ ${this.res.receiptnumber}`,
                        type: 'เงินออก',
                        amount: this.$store.getters.order_insurance_total,
