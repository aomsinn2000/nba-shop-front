            ...
            <div class="col-12">
                ...
                <div class="grid mb-0 pb-0">
                    <div class="col-12 mt-2">
                        <PrintReceipt :order="res" :price="item_price" :tax="tax" className="w-full" />
                    </div>
                </div>
                <div class="grid">
...
import PrintReceipt from '@/components/employee/pos/artwork/PrintReceipt.vue'
export default {
    components: {
        PrintReceipt
    },
    ...
    methods: {
        ...
        checkPaymentType() {
            ...
            if (this.payment_type === 'เงินโอน' || this.payment_type === 'บัตรเครดิต') {
                this.receive = this.$store.getters.order_artwork_total;
            } else {
                this.receive = null;
            }
        },
        check() {
            ...
            if (this.receive < this.$store.getters.order_artwork_total) {
            ...
            console.log(this.$store.getters.order_artwork)

            const count = this.$store.getters.order_artwork_detail.length - 1;
            for (let i = 0; count > i; i++) {
                this.name = this.$store.getters.order_artwork_detail[i].code;
                if (this.name !== 'shipping') {
                    const total = this.$store.getters.order_artwork_detail[i].total;
                    this.total = this.total + total;
                }
            }
            this.tax = this.total;
            console.log('ราคาก่อนหักภาษี : ', this.tax);
            ...
            this.$confirm.require({
                ...
                accept: async () => {
                    ...
                    //check address
                    let address = 'ไม่มี';
                    if (this.$store.getters.artwork_cus.address !== '') {
                        address = this.$store.getters.artwork_cus.address;
                    }
                    this.change = this.receive - this.$store.getters.order_artwork_total;
                    const data = {
                        customer_contact: this.$store.getters.artwork_cus.tel,
                        customer_name: this.$store.getters.artwork_cus.name,
                        customer_tel: this.$store.getters.artwork_cus.tel_platform,
                        customer_address: address,
                        customer_iden_id: this.$store.getters.artwork_cus.people_id,
                        customer_line: this.$store.getters.artwork_cus.line_id,
                        product_detail: this.$store.getters.order_artwork,
                        paymenttype: this.payment_type,
                        moneyreceive: this.receive,
                    }
                    await axios.post(`${process.env.VUE_APP_SHOP}artwork/product-graphic/order`, data, {
                        headers: {
                            'auth-token': `Bearer ${this.$store.getters.token}`,
                        ...
                        this.item_price = this.$store.getters.order_artwork_detail;
