    ...
    methods: {
        ...
        checkPaymentType() {
            ...
            if (this.payment_type === 'เงินโอน' || this.payment_type === 'บัตรเครดิต') {
                this.receive = this.$store.getters.order_itsupport_total;
            } else {
                this.receive = null;
            }
        },
        check() {
            ...
            this.$confirm.require({
                ...
                accept: async () => {
                    ...
                    //check address
                    let address = 'ไม่มี';
                    const num = Number(this.$store.getters.order_itsupport_total);
                    const del_wallet = this.partner_wallet - num;
                    const _id = this.partner._id;
                    if (this.$store.getters.itsupport_cus.address !== '') {
                        address = this.$store.getters.itsupport_cus.address;
                    }
                    this.change = this.receive - this.$store.getters.order_itsupport_total;
                    const data = {
                        customer_tel: this.$store.getters.itsupport_cus.tel_platform,
                        customer_name: this.$store.getters.itsupport_cus.name,
                        customer_contact: this.$store.getters.itsupport_cus.tel,
                        customer_address: address,
                        people_id: this.$store.getters.itsupport_cus.people_id,
                        line_id: this.$store.getters.itsupport_cus.line_id,
                        product_detail: this.$store.getters.order_itsupport,
                        paymenttype: this.payment_type,
                        moneyreceive: this.receive,
                    }
                    await axios.post(`${process.env.VUE_APP_SHOP}itsupportservice/order`, data, {
                        headers: {
                            'auth-token': `Bearer ${this.$store.getters.token}`
                        }
                    }).then((res) => {
                    ...
                    const history = {
                        ...
                        name: `บริการ IT-Support ใบเสร็จ เลขที่ ${this.res.receiptnumber}`,
                        type: 'เงินออก',
                        amount: this.$store.getters.order_itsupport_total,
        ...
        again() {
            ...
            this.$store.commit('setitsupportCus', cus);
            window.location.reload();
        }
    }
}
