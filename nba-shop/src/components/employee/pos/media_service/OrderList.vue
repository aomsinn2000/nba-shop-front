<template>
    <div>
        <Panel>
            <template #header>
                <p class="p-0 m-0"><strong>ทั้งหมด : </strong> {{ $store.getters.order_media_detail.length }} รายการ</p>
            </template>
            <div class="grid">
                <div class="col-12">
                    <DataTable :value="$store.getters.order_media_detail" :rows="5" :paginator="true">
                        <template #empty>
                        ...
                        <Column header="ตัวเลือก" style="width: 15%">
                            <template #body="item">
                                <Button class="p-button-danger p-button-outlined p-button-sm" icon="pi pi-trash"
                                    @click="delWeb(item.index)" v-if="item.data.code === 'media_service'" />
                            </template>
                        </Column>
                    </DataTable>
                </div>
    ...
    methods: {
        delWeb(position) {
            this.$confirm.require({
                ...
                rejectLabel: 'ยกเลิก',
                accept: () => {
                    this.$store.dispatch('delMediaService', position);
                }
            });
        }
    },
